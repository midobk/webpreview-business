...
> Cherry-picked glue so this framework applies cleanly to *this* repo's prototype pipeline. The body above is the canonical framework; the notes below adapt it to Seaway Sites. See `docs/PROTOTYPE_GENERATION_PLAYBOOK.md`, `docs/PROTOTYPE_QA.md`, and `docs/PROTOTYPE_RULES.md` for the authoritative gates. Pair with `.agents/skills/frontend-design/SKILL.md` (aesthetic direction + implementation contract) and `.agents/skills/color-expert/SKILL.md` (palette/contrast).

**The output is one self-contained file, not a Next.js project.** A prototype is a single `data/prototypes/<slug>/index.html` with an inline `<style>` and local images under `images/` — no build step, no external CSS/JS/font CDNs, no framework. It renders inside a sandboxed iframe (`sandbox="allow-scripts"`). So the 7 context artifacts collapse into the **handoff record** (playbook §Handoff record): capture the *visual thesis* and *page job* there instead of separate brief/PRD/tasks files, and the "living style guide" becomes the inline `:root` token system, not a `style-guide.md`.

**Component resources are pattern inspiration, not installable dependencies.** shadcn / 21st.dev / Three.js / Sketchfab are references for *how a pattern looks and behaves* — you hand-inline the equivalent HTML/CSS. Never add a CDN link, `@import`, npm dependency, or framework runtime; they violate the iframe CSP.

**Personalization discipline (hard rule).** Personalize from **verified lead data only** — business name, industry, city, real services, real description. **Never invent** testimonials, awards, reviews, years-in-business, addresses, phone numbers, pricing, guarantees, ratings, or certifications. Fabricated proof is an automatic reject (`PROTOTYPE_RULES` §No False Claims). Every page carries the watermark + demo lock and no CTA performs a real action.

**REVIEW step = the showcase self-score.** Before marking `showcase_eligible`, score 0–5 on each of: visual hierarchy, industry specificity, copy quality, imagery, responsive behavior, accessibility, interaction polish, technical cleanliness. **Any blocker fails outright.** Target **≥32/40**. The automated `scripts/showcase-score/score_showcase.py` gate only checks a page is *not broken* — it never checks that it is *good*. That judgment is yours. **Fail closed:** never publish a generic fallback or placeholder because a dependency timed out (`PROTOTYPE_QA.md` §What NOT to do).

**Per-industry section patterns** (restaurant, salon, contractor, cleaning, auto, generic) live in `docs/PROTOTYPE_RULES.md` — use them as the expected-section checklist, then art-direct beyond them.
