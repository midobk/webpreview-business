...
- [x] **Phase 3** — Lead discovery: 2 scripts (`discover.py` browser-based, `discover_places.py` Google Places API). **1460 leads in `data/leads.json` as of 2026-07-16** (was 170 on 7/14; +1290 new `discovered` status from a large re-run, likely the §19-J-unblocked discovery sweep after the real `GOOGLE_PLACES_API_KEY` was provided or the run-with-env.sh wrapper was wired in). Google Places API live via free trial ($415 credit, expires Sept 21, 2026).
- [x] **Phase 4** — Lead scoring: `score.py` with 10 criteria, 0-100 scale. Statuses auto-assigned. Decisions logged to `logs/decisions.md`. Current breakdown (2026-07-16): 10 ready_for_prototype, 19 pending_review, 123 flag_for_review, 14 email_drafted_pending_email, 3 email_drafted, 1290 discovered, 1 ignore.
- [x] **Phase 5** — Prototype generation: `generate.py` rewritten (now actually calls image_generate + MiniMax M3). **29 prototypes in `data/prototypes/` (28 completed, 1 pending) as of 2026-07-16** — was 24 on 7/14; +5 added in 7/15–7/16 (the 1460-lead discovery surfaced more ready_for_prototype candidates, of which 5 prototypes were generated in this window). Screenshot pipeline working via `scripts/screenshot/screenshot-prototype.js` (file:// Playwright, no dev server). Screenshots live in `public/prototype-screenshots/`. **Showcase snapshot 2026-07-16:** 11 approved, 29 eligible, 19 of 29 have `screenshot_url` (the 10 missing are the recent discovery-batch entries that ship with anonymized-vector SVG thumbnails or are still pending screenshot capture).
...
> **Last audit: 2026-07-17 08:01 EDT** (daily plan-maintenance cron). **Prior audits:** 2026-07-16 08:01 EDT, 2026-07-15 15:28 EDT, 2026-07-14 08:07 EDT, 2026-07-14 08:01 EDT, 2026-07-08 08:01 EDT (plan-maintenance), 2026-07-07 08:01 EDT, 2026-07-06 08:01 EDT, 2026-07-06 02:44 EDT, 2026-07-04 08:52 EDT, 2026-07-03 11:14 EDT, 2026-07-02 08:01 EDT, 2026-07-01 08:01 EDT, 2026-06-30 08:01 EDT, 2026-06-29 10:08 EDT, 2026-06-29 09:55 EDT, 2026-06-26 08:01 EDT, 2026-06-24 15:24 EDT. **Note:** 7/9–7/13 daily audits were skipped (cron fired but produced no edits/row this week — likely because counts hadn't changed between 7/8 and the 7/10 showcase-pass work, and the maintenance cron appears to have stopped running mid-week until the user nudged it back).
>
> **Cron jobs:** ⚠️ **1 in live scheduler** (this `sitesprint-agent-plan-maintenance` job only — re-verified 2026-07-14 08:01 EDT: `cron action=list` returns `total: 1, hasMore: false`). The 6 other crons (prototype-generation, email-drafting, discovery-run1/2/3, sitesprint-weekly-planning) are NOT scheduled. §19-I still open. DO NOT mark §19-I DONE again until `cron action=list` returns 7.
>
> **Drift fixed 2026-07-14 08:01 EDT (THIS RUN):** (1) Lead count 168 → **170** (+2 new `discovered` status since 7/8); (2) Prototype count 15 → **24** (+9 added: 6 high-score premium-saas + 3 anonymized-public-preview upgrades for Bella's/Seaway/Premium Cleaning); (3) Showcase approved 2 → **11**, eligible 10 → **24**; (4) Phase 5 working-copy bullet's stale "2 approved / 10 eligible" → "11 approved / 24 eligible"; (5) PR #3 status: **MERGED** 2026-06-25 19:35 UTC via `c5164bc` — file still said OPEN; flipped to DONE; (6) New open PR detected: PR #4 `claude/landing-page-design-ixddwp` "Seaway Sites V2" (opened 2026-07-13 08:10 UTC) — added to Open Work J.1. MEMORY.md C1–C4 still tracked in §17 as DONE. Pricing in `app/page.tsx` still matches §3 (verified lines 86, 154-155, 171, 187).
>
> **New work captured 2026-07-14 08:01 EDT:** (1) 5 daily audits between 7/9 and 7/13 were skipped; (2) `scripts/run-with-env.sh` wrapper added in commit `9bf49f3` to source `.env.local` for cron jobs — this is exactly the §19-J option (b) the user was asked to pick. **Mark §19-J as PARTIALLY DONE** (mechanism exists, needs to be wired into the discovery cron payloads + re-registered per §19-I); (3) PR #4 (landing page design) is the next user-actionable review.
>
