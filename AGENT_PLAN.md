...
#### J.1. PR #4 (landing page design) — awaiting review
- [ ] **PR #4 — `claude/landing-page-design-ixddwp`** "Seaway Sites V2 — approved positioning, pricing, ownership, and trust pass" (opened 2026-07-13 08:10 UTC, branch `claude/landing-page-design-ixddwp`). Need `gh pr view` to inspect diff. Likely affects `app/page.tsx` hero/positioning copy + trust-stats per the 06-23 §18 M7 fix (which was "honest" stats; this PR is the next iteration). **Acceptance:** run `tsc --noEmit` + `next build` against the branch in a worktree, confirm pricing still matches §3 (Free / $299+$49/mo / $500 / $799), confirm no fabricated stats, then user reviews.
