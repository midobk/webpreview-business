...
> **See DRIFT CORRECTION header above for the 2026-06-29 10:08 EDT reconciliation.** This section's other rows below remain accurate except where noted (Cron Jobs line). **Drift fix 2026-07-14 08:01 EDT:** lead count 168→170, prototype count 15→24, showcase approved 2→11, eligible 10→24, PR #3 status OPEN→MERGED. See run log row + §19 J.1.
...
- [x] **Phase 3** — Lead discovery: 2 scripts (`discover.py` browser-based, `discover_places.py` Google Places API). **170 leads in `data/leads.json` as of 2026-07-14** (was 168 on 7/8; +2 new `discovered` status from showcase work). Google Places API live via free trial ($415 credit, expires Sept 21, 2026).
- [x] **Phase 4** — Lead scoring: `score.py` with 10 criteria, 0-100 scale. Statuses auto-assigned. Decisions logged to `logs/decisions.md`. Current breakdown (2026-07-14): 15 ready_for_prototype, 19 pending_review, 121 flag_for_review, 9 email_drafted_pending_email, 3 email_drafted, 2 discovered (NEW), 1 ignore.
- [x] **Phase 5** — Prototype generation: `generate.py` rewritten (now actually calls image_generate + MiniMax M3). **24 prototypes in `data/prototypes/` (23 completed, 1 pending) as of 2026-07-14** — was 15 on 7/8; +9 added in showcase-expansion: 6 high-score premium-saas entries (catalyst-online-course 91, northwind-ecommerce 92, allied-home-services 93, meridian-law-firm 94, summit-fitness-gym 92, bright-smile-dental 93) + 3 anonymized-public-preview upgrades (Bella's Hair Studio + Seaway Cleaning + premium Cleaning). Screenshot pipeline working via `scripts/screenshot/screenshot-prototype.js` (file:// Playwright, no dev server). Screenshots live in `public/prototype-screenshots/`. **Showcase snapshot 2026-07-14:** 11 approved, 24 eligible, 19 of 24 have `screenshot_url` (the 5 missing are the 6 new premium-saas entries that ship with anonymized-vector SVG thumbnails per `8f00897` / `0049f7c`, not Playwright screenshots).
