...
- [x] **Phase 3** — Lead discovery: 2 scripts (`discover.py` browser-based, `discover_places.py` Google Places API). **~1419 leads in Supabase (source of truth) as of 2026-07-19 02:36 EDT**; local `data/leads.json` now shows 1000 after `da677b2` Supabase re-sync (7/20). Google Places API live via free trial ($415 credit, expires Sept 21, 2026).
- [x] **Phase 4** — Lead scoring: `score.py` with 10 criteria, 0-100 scale. Statuses auto-assigned. Decisions logged to `logs/decisions.md`. Current local-JSON breakdown (2026-07-20): 4 ready_for_prototype, 19 pending_review, 123 flag_for_review, 14 email_drafted_pending_email, 10 prototype_drafted, 3 email_drafted, 2 email_sent, 823 discovered, 1 new, 1 ignore.
- [x] **Phase 5** — Prototype generation: `generate.py` rewritten (now actually calls image_generate + MiniMax M3). **~29 prototypes in Supabase (28 completed, 1 pending) as of 2026-07-19 02:36 EDT**; local `data/prototypes.json` now shows 41 after `da677b2` re-sync + 5 new prototypes generated 7/19 20:47–7/20 02:29 (Prestige Cuts, Gilles' 3rd Gen Barber Shop, POSH NOSH Pastry Confectionery, Roy Florist, Quilles Nativité Bowling). Screenshot pipeline working via `scripts/screenshot/screenshot-prototype.js` (file:// Playwright, no dev server). Screenshots live in `public/prototype-screenshots/`. **Showcase snapshot 2026-07-20:** 11 approved, 32 eligible, 30 of 41 have `screenshot_url` (11 missing are fixture/test entries or 7/19–7/20 batch entries still pending screenshot capture).
...
- **Email** — **Resend** chosen 2026-07-21 (replaces AgentMail). `RESEND_API_KEY` + `RESEND_FROM_ADDRESS` set on Vercel Prod+Preview (2026-07-21), from `Seaway Sites <mehdi@seawaysites.com>`. Lead-reply send path (`scripts/pipeline/send_preview_email.py`) already uses Resend — G2 code-done. Proactive outreach (`scripts/send-outreach`) still targets AgentMail — separate PR. Remaining for true end-to-end: rotate the shared Resend key + verify `seawaysites.com` in Resend (DKIM/SPF).
...
- **Meta ads** (2026-07-21): MCP authenticated against `SeawaySitesAdAccount` (act_1469618098544438, CAD, America/Toronto, fresh, $0 spent). Page = `Seaway Sites` (id 1175228509015042). Pixel `1530207232234428` + CAPI access token both provisioned and set on Vercel Prod+Preview; tracking stack live (browser pixel confirmed in prod HTML, CAPI test event accepted by Meta). Full plan in `docs/META_ADS_PLAN.md`.
